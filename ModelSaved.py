...
from tensorflow.keras.layers import Input, Conv2D, Conv2DTranspose, MaxPooling2D, UpSampling2D, Dense, Flatten, Lambda, Reshape

class ModelSaved():

    ## Set the constants and paths
    def __init__(self, modelSel, imageDim, intermediateDim = 64):
        ...
        try:
            if self.modelName == 'VAE_C1':
                model = self.build_vaeC1_model()

            elif self.modelName == 'VAE_F1':
                model = self.build_vaeF1_model()

            elif self.modelName == 'BAE1':
                model = self.build_bae1_model()

            elif self.modelName == 'BAE2':
                model = self.build_bae2_model()

            elif self.modelName == 'MVT':
                model = self.build_mvt_model()
    ...
    def build_vaeC1_model(self):
        ...
        input_img = Input(shape = (self.height, self.width, self.channel))
        ...
        z_log_sigma = Dense(self.latent_dim,name="z_log_sigma")(h)
        z_mean = Dense(self.latent_dim,name="zmean")(h)
    ...
    def build_bae_model_old(self):
        ...
        net = UpSampling2D((2, 2))(net)
        decoded = Conv2D(self.channel, (3, 3), activation='sigmoid', padding='same')(net)
        # ---------------------------------------------------------------------
    ...
    def build_mvt_model(self):
        ...
        input_img = Input(shape=(self.height, self.width, self.channel))  # adapt this if using `channels_first` image data format
        ...
        x = Conv2D(32, (4, 4), strides=2, activation='relu', padding='same')(x)
        x = UpSampling2D((2, 2))(x)
        decoded = Conv2D(self.channel, (8, 8), activation='sigmoid', padding='same')(x)
        # ---------------------------------------------------------------------
